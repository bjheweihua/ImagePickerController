...
- (void)viewDidLoad {
    ...
    _showTakePhotoBtn = (([[UIImageManager manager] isCameraRollAlbum:_model.name]) && tzImagePickerVc.allowTakePicture);
    if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
        [[UIImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(UIAlbumModel *model) {
            _model = model;
        ...
        if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
            [[UIImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<UIAssetModel *> *models) {
...
-(void) initNaviBar {
    ...
    [titleLabel setText:_model.name];
...
//    if (iOS7Later) top += 20;
    ...
    _collectionView = [[JRCollectionView alloc] initWithFrame:CGRectMake(0, top, self.view.tz_width, collectionViewHeight) collectionViewLayout:layout];
    ...
    [_collectionView registerClass:[UIAssetCell class] forCellWithReuseIdentifier:@"UIAssetCell"];
...
- (void)configBottomToolBar {
    ...
    UIToolbar *bottomToolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 50, self.view.tz_width, 50)];
    bottomToolBar.tintColor = [UIColor colorWithWhite:1.0 alpha:0.3];
    bottomToolBar.translucent = YES;
    
    CGFloat previewWidth = [tzImagePickerVc.previewBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil].size.width + 2;
    ...
    [_previewButton setTitleColor:[UIColor jrColorWithHex:@"#666666"] forState:UIControlStateNormal];
    [_previewButton setTitleColor:[UIColor jrColorWithHex:@"#cccccc"] forState:UIControlStateDisabled];
...
//        [_originalPhotoButton setTitleColor:[UIColor blackColor] forState:UIControlStateSelected];
        [_originalPhotoButton setTitleColor:[UIColor jrColorWithHex:@"#666666"] forState:UIControlStateNormal];
        [_originalPhotoButton setTitleColor:[UIColor jrColorWithHex:@"#cccccc"] forState:UIControlStateDisabled];
        ...
        _originalPhotoLable.font = [UIFont systemFontOfSize:14];
...
- (void)previewButtonClick {
    UIPhotoPreviewController *photoPreviewVc = [[UIPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    [UIImageManager manager].shouldFixOrientation = YES;
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        UIAssetModel *model = tzImagePickerVc.selectedModels[i];
        [[UIImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"UIAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        UIImagePickerNaviController *tzImagePickerVc = (UIImagePickerNaviController *)weakSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            weakCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (UIAssetModel *model_item in selectedModels) {
                if ([[[UIImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[UIImageManager manager] getAssetIdentifier:model_item.asset]]) {
        ...
        [UIView showOscillatoryAnimationWithLayer:weakLayer type:UIOscillatoryAnimationToSmaller];
    };
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIAssetModel *model = _models[index];
    if (model.type == UIAssetModelMediaTypeVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            UIImagePickerNaviController *imagePickerVc = (UIImagePickerNaviController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            UIVideoPlayerController *videoPlayerVc = [[UIVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == UIAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            UIImagePickerNaviController *imagePickerVc = (UIImagePickerNaviController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            UIGifPhotoPreviewController *gifPreviewVc = [[UIGifPhotoPreviewController alloc] init];
...
- (void)pushPhotoPrevireViewController:(UIPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    UIImagePickerNaviController *imagePickerVc = (UIImagePickerNaviController *)self.navigationController;
    [[UIImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (UIAssetModel *model in _models) {
        ...
        UIImagePickerNaviController *tzImagePickerVc = (UIImagePickerNaviController *)self.navigationController;
        for (UIAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)dealloc {
    //NSLog(@"UIPhotoPickerController dealloc");
}

#pragma mark - Asset Caching

- (void)resetCachedAssets {
    [[UIImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[UIImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                            targetSize:AssetGridThumbnailSize
                                           contentMode:PHImageContentModeAspectFill
                                               options:nil];
        [[UIImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                           targetSize:AssetGridThumbnailSize
                                          contentMode:PHImageContentModeAspectFill
                                              options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            UIAssetModel *model = _models[indexPath.item];
