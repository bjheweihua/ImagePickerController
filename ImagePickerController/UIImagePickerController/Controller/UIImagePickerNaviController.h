...
/*
 经过测试，比起xib的方式，把UIAssetCell改用纯代码的方式来写，滑动帧数明显提高了（约提高10帧左右）
...
@protocol UIImagePickerNaviControllerDelegate;
@interface UIImagePickerNaviController : UINavigationController
...
@property (nonatomic, strong) NSMutableArray<UIAssetModel *> *selectedModels;
...
@property (nonatomic, weak) id<UIImagePickerNaviControllerDelegate> pickerDelegate;

@end


@protocol UIImagePickerNaviControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these handle will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
...
// 你可以通过一个asset获得原图，通过这个方法：[[UIImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(UIImagePickerNaviController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
- (void)imagePickerController:(UIImagePickerNaviController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos;
- (void)imagePickerControllerDidCancel:(UIImagePickerNaviController *)picker __attribute__((deprecated("Use -tz_imagePickerControllerDidCancel:.")));
