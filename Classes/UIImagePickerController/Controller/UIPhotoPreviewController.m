...
#import "UIPhotoPreviewCell.h"
#import "UIAssetModel.h"
#import "UIView+Layout.h"
#import "UIImagePickerNaviController.h"
#import "UIImageManager.h"
#import "UIImageCropManager.h"
#import "UIColor+Additions.h"

@interface UIPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation UIPhotoPreviewController
...
- (void)viewDidLoad {
    ...
    UIImagePickerNaviController *_tzImagePickerVc = (UIImagePickerNaviController *)weakSelf.navigationController;
...
- (void)configCustomNaviBar {
    ...
    _naviBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, self.view.tz_width, 64)];
    _naviBar.tintColor = [UIColor colorWithWhite:1.0 alpha:0.3];
    _naviBar.translucent = YES;
...
- (void) initCustomNaviBar {
    ...
    _backButton.frame = CGRectMake(0, 10, image.size.width, image.size.height);
    _backButton.backgroundColor = [UIColor clearColor];
    [_backButton setImage:image forState:UIControlStateNormal];
    [_backButton setImage:imagePressed forState:UIControlStateHighlighted];
    [_backButton addTarget:self action:@selector(backButtonClick) forControlEvents:UIControlEventTouchUpInside];
    
    _selectButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.tz_width - 54, 10, 42, 42)];
    [_selectButton setImage:[UIImage imageNamedFromMyBundle:@"photo_unchoose"] forState:UIControlStateNormal];
    [_selectButton setImage:[UIImage imageNamedFromMyBundle:@"photo_choose"] forState:UIControlStateSelected];
    [_selectButton addTarget:self action:@selector(select:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)configBottomToolBar {
    ...
    UIImagePickerNaviController *_tzImagePickerVc = (UIImagePickerNaviController *)self.navigationController;
...
//        [_originalPhotoButton setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        ...
        _originalPhotoLable.textColor = [UIColor jrColorWithHex:@"#666666"];
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[UIPhotoPreviewCell class] forCellWithReuseIdentifier:@"UIPhotoPreviewCell"];
}
...
- (void)configCropView {
    ...
    if (!_tzImagePickerVc.showSelectBtn && _tzImagePickerVc.allowCrop) {
        ...
        [UIImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)select:(UIButton *)selectButton {
    UIImagePickerNaviController *_tzImagePickerVc = (UIImagePickerNaviController *)self.navigationController;
    UIAssetModel *model = _models[_currentIndex];
        ...
        for (UIAssetModel *model_item in selectedModels) {
            if ([[[UIImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[UIImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    UIAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:UIOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:UIOscillatoryAnimationToSmaller];
}
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        UIPhotoPreviewCell *cell = (UIPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [UIImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [UIImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UIPhotoPreviewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"UIPhotoPreviewCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[UIPhotoPreviewCell class]]) {
        [(UIPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)dealloc {
    //NSLog(@"UIPhotoPreviewController dealloc");
}
...
- (void)showPhotoBytes {
    [[UIImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
