...
@interface UIPhotoPreviewCell ()
@end

@implementation UIPhotoPreviewCell
...
- (void)setModel:(UIAssetModel *)model {
    ...
    if (model.type == UIAssetModelMediaTypePhotoGif) {
        [[UIImageManager manager] getOriginalPhotoDataWithAsset:model.asset completion:^(NSData *data, NSDictionary *info, BOOL isDegraded) {
            if (!isDegraded) {
...
- (void)setAsset:(id)asset {
    _asset = asset;
    [[UIImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
